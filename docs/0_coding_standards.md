...
`docs/7_leaderboard_improvement_insights.md`'s running ledger). Current docs:

- `0_coding_standards.md`
- `1_instructions.md`
- `2_eda_insights.md`
- `3_baseline_modeling.md`
- `4_implementation_plan.md`: the original day-1 plan, kept for its starting
  intent (merged in the day-1 model-optimization plan, originally a
  separate doc, for the same reason — both describe a `03`/`04` notebook
  pair that was never built).
- `5_submission_manifest.md`
- `6_submission_quota_strategy.md`
- `7_leaderboard_improvement_insights.md`
- `8_v20_target_encoding_plan.md`: a pre-written implementation spec, not a
  result doc — v20 was rejected like every other post-v8 experiment
  (`docs/7`). It exists because target encoding is target-derived and
  ...
  swap), which needed only a `docs/7` ledger entry. Reserve a further
  `N_*_plan.md` doc only when a future experiment needs this kind of
  pre-implementation spec — most rejected OOF variants belong in `docs/7`'s
  running ledger instead.

Retired: a former `8_output_insights_next_steps.md` duplicated `docs/3`'s
v3/v5 validation tables under a different narrative framing and was folded
into `docs/3`; a former `4_model_optimization_and_ensemble.md` duplicated
`docs/4`'s (formerly `docs/5`'s) modeling-strategy bullets and was folded in
above. Both retirements happened before the 2026-07-20 renumbering that
closed the resulting gaps, so no number was permanently skipped.
...
- Keep feature engineering fold-safe: target-derived transformations
  ...
  training fold only — see `docs/8_v20_target_encoding_plan.md` for the
...
- `build_domain_features` and `build_geometry_forge` in
  ...
  `docs/8_v20_target_encoding_plan.md` §2 for why it needs its own
...
the general rule, and `docs/6_submission_quota_strategy.md`'s
"Notebook-First Submission Rule" for this project's exact procedure and what
to record.
